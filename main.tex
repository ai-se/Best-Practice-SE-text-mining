...
Prior to BigSE,
the baseline LexisNexis text miner for 2Class data was:
\bi
...
\textbf{Normalization on columns} transforms a  value $x$ in a column to $(x - \mathif{min})/(\mathit{max} - \mathit{min})$ (where \mathit{min}, \mathit{max} come
for that column).
